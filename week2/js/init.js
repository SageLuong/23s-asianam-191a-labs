...
var smileyicon = L.icon({
    ...
    shadowAnchor: [8, 35],  // the same for the shadow
...
function addMarker(lat,lng,title,image,message, link){
    console.log(message)
    L.marker([lat,lng], {icon: smileyicon}).addTo(map).bindPopup(`<center> <h2>${title}</h2> </center> <center> <img src="${image}" height="150px" width="150px"> </center> <h3>${message}</h3> <h3> Take a look at their <a href="${link}"> menu! </a></h3>` )
    return message
}
